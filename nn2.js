...
class network {
    ...
    SGD (tr_data ,  vdata , epochs  , mini_batch  , eta , lambda){
        ...
        for  (let  e  =  0 ; e  < epochs; e ++ ){ //will train  our model for  epochs time hhhhh 
           ...
           // console.log(this.feedforward(tr_data[0][0]) ,  tr_data[0][1])
            ...
            for  ( let  j  =  0 ;  j  < mini_batchs.length ; j++ ){
                ...
                this.update_mini_batch( mini_batchs [j] ,  eta , lambda , tr_data.length);
            }
    ...
    update_mini_batch  ( mini_batch ,  eta , lambda , n ){
        ...
        for (let  l  = 0 ;  l  < nabla_w.length ;  l ++ ){
            nabla_w[l] = ematrix( this.weights[l], nabla_w[l] ,
                (a ,b) => (1  -  (eta*lambda)/n)*a  - (eta * (b /mini_batch.length))) 
        }
    ...
    backprop  (x  , y ){
        ...
        let delta  =  prvector(
            this.cost_derivative(activetions[activetions.length -1] , y) , 
